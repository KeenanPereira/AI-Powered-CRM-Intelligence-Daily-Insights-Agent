...
with tab1:
    ...
    with col_b:
        pipeline = d["pipeline"].get("pipeline_statuses", {})
        if pipeline:
            df_funnel = (
                pd.DataFrame(list(pipeline.items()), columns=["Stage", "Count"])
                  .sort_values("Count", ascending=False)
            )
            fig = px.funnel(
                df_funnel, x="Count", y="Stage",
                title="🔻 Unified Conversion Funnel (Leads + Deals)"
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
...
with tab2:
    ...
    if sq:
        ...
        df_tree = pd.DataFrame([
            {"Source": src, "Total": m["total_leads"], "Junk %": m["junk_pct"]}
            for src, m in sq.items()
        ])
        fig2 = px.treemap(
            df_tree, path=["Source"], values="Total", color="Junk %",
            color_continuous_scale=["#00CC96", "#FFA15A", "#EF553B"],
            title="🗺️ Lead Volume Treemap — Colored by Junk %"
        )
...
with tab5:
    ...
    with col_a:
        ...
        if ai_dates:
            ...
            if briefing:
                st.info("Generated by local Mistral 7B — grounded strictly in Supabase SQL data.")
...
st.divider()
st.caption("Powered by Zoho CRM · Supabase Cloud PostgreSQL · Streamlit · Mistral 7B (Local)")
